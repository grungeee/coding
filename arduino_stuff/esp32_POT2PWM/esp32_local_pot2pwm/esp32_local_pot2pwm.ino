/*
  Standalone ESP32 potentiometer to PWM converter.

  Local analog inputs:
    pot1 -> GPIO32
    pot2 -> GPIO34
    pot3 -> GPIO35
    switch -> GPIO33, system is ON when not closed to GND
...
void setup() {
  ...
  delay(1000);
  Serial.println();
  Serial.println("BOOT: Standalone ESP32 pot-to-PWM");
  setupEspNow();

  ledcAttach(pwm1Pin, pwmFreq, pwmResolution);
  ledcAttach(pwm2Pin, pwmFreq, pwmResolution);
  ledcAttach(pwm3Pin, pwmFreq, pwmResolution);
  ledcAttach(switchOut1Pin, pwmFreq, pwmResolution);
  ledcAttach(switchOut2Pin, pwmFreq, pwmResolution);
  ledcAttach(switchOut3Pin, pwmFreq, pwmResolution);

  pinMode(switchPin, INPUT_PULLUP);
  ledcWrite(switchOut1Pin, 0);
  ledcWrite(switchOut2Pin, 0);
  ledcWrite(switchOut3Pin, 0);
...
void loop() {
  ...
  uint16_t pot1Raw = analogRead(pot1Pin);
  uint16_t pot2Raw = analogRead(pot2Pin);
  uint16_t pot3Raw = analogRead(pot3Pin);
  bool switchRaw = digitalRead(switchPin) == HIGH;
  ...
  ledcWrite(pwm1Pin, pwm1);
  ledcWrite(pwm2Pin, pwm2);
  ledcWrite(pwm3Pin, pwm3);
  ledcWrite(switchOut1Pin, switchDuty);
  ledcWrite(switchOut2Pin, switchDuty);
  ledcWrite(switchOut3Pin, switchDuty);
